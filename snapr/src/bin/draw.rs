use image::*;
// use opencv::{imgproc::{resize, INTER_LINEAR}, core::Size};
...
fn tick<R>(name: &str, f: impl Fn() -> R) -> R {
    ...
    let result = f();
    println!("[{}] elapsed time: {}s", name, now.elapsed().as_secs_f32());
    result
}
...
fn main() {
    ...
    // image = image::imageops::resize(&image, image.width() / 2, image.height() / 2, image::imageops::FilterType::Nearest);

    let matrix = ImageMatrix::from_image(&image);

    // let detector = FaceDetector::default();
    let cnn_detector = FaceDetectorCnn::default();
    let landmarks_pred = LandmarkPredictor::default();
    ...
    for r in face_locations.iter() {
        ...
        let landmarks = tick("LandmarkPredictor", || {
            landmarks_pred.face_landmarks(&matrix, &r)
        });
